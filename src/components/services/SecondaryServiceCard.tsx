...
export function SecondaryServiceCard({ service }: SecondaryServiceCardProps) {
  const content = (
    <div className="bg-white border border-stone-100 rounded-xl px-5 py-4 hover:border-primary/20 hover:shadow-md transition-[border-color,box-shadow] duration-300 group h-full">
      <div className="flex items-center gap-3 mb-2">
        <service.Icon className="w-5 h-5 text-primary/70 group-hover:text-primary transition-colors" strokeWidth={1.5} />
        <h3 className="text-stone-900 text-sm">{service.title}</h3>
      </div>

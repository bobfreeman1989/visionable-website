...
export default function Process() {
  return (
    <section id="process" className="py-14 bg-[#FAFAF8]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {steps.map((s, i) => (
            <FadeUp key={s.num} delay={i * 0.1}>
              <div className="relative bg-[#FEFEFE] rounded-2xl p-6 border border-gray-200 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 h-full group">
                {/* Connector line */}
                {i < steps.length - 1 && (
                  <div className="hidden lg:block absolute top-10 -right-3 w-6 border-t-2 border-dashed border-primary/30" />
                )}
                <div className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-primary text-white text-sm font-bold mb-3 group-hover:bg-accent transition-colors duration-300">
                  {s.num}
                </div>
